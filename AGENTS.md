...
- The latest LongLive2 Blackwell test used `RTX 5090 x1` / `sm120` with `nvfp4_s2`, `sp_size=1`, `dp_size=1`, and `sampling_steps=2`. It built, rendered, published R2, and tore down cleanly, but it did not hit realtime at `480x832`.
- Latest RTX 5090 LongLive2 result: nonblank `832x480`, `125` frame MP4 at playback `24 fps`; generation was `wall_video_fps=1.151356` and `wall_render_fps=0.294747`, so `wall_fps_ok=false`.
- Do not rent more than one B200/GB200 for the Blackwell LongLive2 lane; only use B200/GB200 if a sane one-GPU offer appears.
...
- Latest LongLive2 RTX 5090 SM120 tuple `longlive2_nvfp4_s2_py312_torch2.10.0_cu128_sm120_prebuild1` is a `published_tuple`, not a `validated_restore_tuple`; run a fresh restore-only validation before using it as a reusable fast path.
...
- `VideoDiffusion/run_longlive2_sp_vast_smoke.sh --blackwell-tier sm120 --blackwell-cold-build` is the RTX 5090 NVFP4 cold-build path; it sets one-GPU `nvfp4_s2` defaults, `CUDA_ARCHS=120`, constrained Blackwell source-build settings, relaxed one-GPU offer search, and Blackwell R2 publish metadata.
- Blackwell success must be judged by wall-clock render FPS from `run_timing.json` / `run_report.json`, not MP4 playback FPS. The Blackwell wrapper sets `--min-wall-fps 24` so `run_report.json` acceptance fails when generation is below realtime.
- For Blackwell cold builds, R2 publish eligibility is intentionally separate from realtime acceptance: publish only after a valid nonblank render with telemetry and no torch errors, then mark the run failed if `wall_fps_ok` is false. This preserves useful build/cache artifacts without promoting a slow run as realtime.
- The wrapper launches long LongLive2 remote phases as detached logged steps with bounded SSH launch timeout and status polling. Keep that behavior; it was added after a local SSH launch hung while the remote host was still usable.
