...
def has_arch_90() -> bool:
    arch_list = os.getenv("TORCH_CUDA_ARCH_LIST", "")
    if not arch_list:
        return False
    normalized = arch_list.replace(";", ",").replace(" ", "")
    return "sm_90" in normalized or "90" in normalized.replace("sm_", "")
...
def _write_ninja_file(
    ...
    **kwargs,  # kwargs (ignored) to absorb new flags in torch.utils.cpp_extension
    ...
    if with_cuda:
        ...
        # When sm90 builds are intentionally skipped we must prevent implicit sm90
        # codegen from any unsuffixed CUDA source rule.
        raw_skip_sm90 = os.getenv("MAGI_ATTENTION_SKIP_SM90", "AUTO").strip().upper()
        if raw_skip_sm90 in {"1", "TRUE", "YES", "ON", "Y"}:
            skip_sm90 = True
        elif raw_skip_sm90 in {"0", "FALSE", "NO", "OFF", "N"}:
            skip_sm90 = False
        else:
            skip_sm90 = not has_arch_90()

        effective_cuda_post_cflags = [
            "arch=compute_80,code=sm_80" if s == "arch=compute_90a,code=sm_90a" else s
            for s in cuda_post_cflags
        ]

        flags.append(f'cuda_post_cflags = {" ".join(effective_cuda_post_cflags)}')
...
if not SKIP_CUDA_BUILD:
    ...
    if False: # <<< ADDED: Prevent download logic execution
        if bare_metal_version != Version("12.8"):
            download_and_copy(
                ...
                url_func=lambda system, arch, version: f"https://developer.download.nvidia.com/compute/cuda/redi\
st/cuda_nvcc/{system}-{arch}/cuda_nvcc-{system}-{arch}-{version}-archive.tar.xz",  # noqa: E501
            )
            download_and_copy(
                name="ptxas",
...
st/cuda_nvcc/{system}-{arch}/cuda_nvcc-{system}-{arch}-{version}-archive.tar.xz",  # noqa: E501
    ...
    if skip_sm90:
        ...
        sources_bwd_sm90 = []
        if not DISABLE_SM8x:
            print("[magi-attention] MAGI_ATTENTION_SKIP_SM90=1: skipping Sm90 kernels.")
        else:
            print("[magi-attention] MAGI_ATTENTION_SKIP_SM90=1: using Sm80-only kernel set.")

    sources = (
        [f"{ffa_dir_rel}/flash_api.cpp"]
        + (sources_fwd_sm80 if not DISABLE_SM8x else [])
        + ([] if skip_sm90 else sources_fwd_sm90)
        + (sources_bwd_sm80 if not DISABLE_SM8x else [])
        + ([] if skip_sm90 else sources_bwd_sm90)
    )
