...
- `scripts/cloudflare/prebuild_bundle.py` + `VideoDiffusion/publish_r2_prebuild_model.sh` + `VideoDiffusion/restore_r2_prebuild_model.sh` — runtime tuple publish/restore pipeline (`--mode auto|tuple|image`) for MAGI and Krea.
