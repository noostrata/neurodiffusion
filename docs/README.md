...
- latest Scope/LongLive edge sweep: same-instance H200 run passed `352x576` at `24.835 fps` and failed `368x640` at `22.175 fps`; run root `/Users/xenochain/Downloads/scope_longlive_vast_smoke_20260520T211512Z/`.
...
- latest LongLive2 SP status: H200 x2 BF16 SP cold render succeeded and produced `/Users/xenochain/Downloads/longlive2_sp_vast_smoke_20260520T233039Z/offline/videos/rank0-0-0_regular_sp2.mp4` (`832x480`, `125` frames, `24 fps`, nonblank QA). The run published `longlive2_bf16_sp_py310_torch2.8.0_cu128_sm90_prebuild1` to R2. First fresh restore validation fetched/extracted the tuple but failed before render because the restored cache did not recreate the LongLive2 Wan symlink; `VideoDiffusion/restore_r2_prebuild_model.sh` now patches that, so the next paid step is one no-fallback restore render before promoting the tuple to validated.
