...
- latest LongLive2 SP status: H200 x2 BF16 SP cold render succeeded, the BF16 SM90 tuple was published to R2, and fresh H100 NVL x2 restore validation succeeded. The decisive benchmark-only run at `/Users/xenochain/Code/neurodiffusion/artifacts/runs/longlive2/longlive2_sp_vast_smoke_20260521T114258Z/` measured `sp2` slower than `sp1` (`speedup_sp2_over_sp1=0.663945`), so LongLive2 BF16 SP is research/offline-only for now and Scope/LongLive remains the realtime EEG path.
- next LongLive2 Blackwell target: `RTX 5090 x1` / `sm120` with `nvfp4_s2`; use `VideoDiffusion/run_longlive2_sp_vast_smoke.sh --blackwell-tier sm120 --blackwell-cold-build` after preflight, and judge realtime by wall-clock render FPS, not MP4 playback FPS. Blackwell mode sets a `24 fps` report acceptance gate, while R2 publish eligibility is based on a valid nonblank render so useful cold-build caches are not discarded after a slow first run.
