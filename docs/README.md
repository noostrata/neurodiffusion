...
- next LongLive2 Blackwell target: `RTX 5090 x1` / `sm120` with `nvfp4_s2`; use `VideoDiffusion/run_longlive2_sp_vast_smoke.sh --blackwell-tier sm120 --blackwell-cold-build` after preflight, and judge realtime by wall-clock render FPS, not MP4 playback FPS. Blackwell mode sets a `24 fps` report acceptance gate.
