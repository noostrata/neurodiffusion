...
- **Method**:
    ...
    - One thread generates images using SD-Turbo optimized with `torch.compile` and `xformers`. This thread now reads both prompt and generation parameters from shared, thread-safe state objects.
    - The Flask app serves:
        - `/`: A simple HTML page with an `<img>` tag pointing to `/stream`, a prompt input form, and sliders for `num_inference_steps` and `guidance_scale`.
        - `/stream`: A `multipart/x-mixed-replace` (MJPEG) endpoint continuously yielding the latest generated JPEG frame.
        - `/prompt`: A POST endpoint to receive a new prompt via JSON, updating the shared prompt state.
        - `/params`: GET endpoint to retrieve current parameter values (steps, guidance); POST endpoint to update them via JSON.
...
4.  **Access Stream (Locally):** Open a web browser and navigate to `http://localhost:8888/`.
    - You should see the web UI with the image stream, prompt input, and sliders for "Steps" and "Guidance Scale".
    ...
    - Adjusting the sliders will update the `num_inference_steps` and `guidance_scale` used by the generation thread in near real-time (after a brief debounce period).

### Server Details (`realtime_stream.py`)
- **Configuration:** Key parameters like `TARGET_FPS`, `INITIAL_PROMPT`, `MODEL_ID`, `SERVER_PORT`, `DEFAULT_NUM_INFERENCE_STEPS`, `DEFAULT_GUIDANCE_SCALE` are at the top of the script.
...
- **Generation Thread (`_gen_loop`):** Runs continuously, gets the current prompt and parameters (`num_inference_steps`, `guidance_scale`) from thread-safe state objects (`PromptState`, `ParamState`), generates an image using these values, encodes it to JPEG in memory, stores it in `latest_jpeg`, and paces itself based on `TARGET_FPS`.
- **Flask App:**
    - **`/`:** Serves the basic HTML interface, including the prompt input and parameter sliders. Contains JavaScript to handle slider input and POST updates to `/params`.
    ...
    - **`/prompt`:** Handles POST requests with JSON (`{"prompt": "..."}`), updates the shared `PROMPT_STATE` using a lock, and logs activity.
    - **`/params`:** Handles GET requests to return the current parameters held in `PARAM_STATE`, and POST requests with JSON (`{"num_inference_steps": ..., "guidance_scale": ...}`) to update `PARAM_STATE`.
...
- **Look for:**
    ...
    - `[params] Updated: {'num_inference_steps': ..., 'guidance_scale': ...}` (Successful parameter update via slider)
...
- **Use Developer Tools (F12):** Essential for debugging client-side problems.
    ...
    - **Network Tab:**
        ...
        - Inspect the requests to `/params` (POST) when moving sliders. Status should be 200. Check Request Payload and Response. A 500 status indicates a server-side crash in the `/params` handler (check `server.log`).
...
- **Sliders not working:** Check the browser's JavaScript console (F12 -> Console) for errors related to slider event handlers or the `fetch` calls to `/params`. Verify the `/params` endpoint is reachable and responding correctly (check Network tab and `server.log`).
...
- **Architecture:** Flask + MJPEG streaming is effective for real-time viewing. Separating generation into a thread allows Flask to remain responsive. Shared state objects (`PromptState`, `ParamState`) with locks allow safe communication between Flask routes and the generator thread.
...
- **Debugging:** Check logs systematically: `server.log` (remote) for server errors, tunnel script output (local) for connection issues, and browser developer tools (local) for client-side/network request problems (Console and Network tabs are key).
